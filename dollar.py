...
async def _pre_analyze_next(client):
    """Analyze next signal pairs in background while waiting for result candle."""
    global pending_next_signal
    try:
        await asyncio.sleep(5)  # brief pause after sending signal
        pairs = await asyncio.wait_for(get_profitable_pairs(client, min_profit=70), timeout=15)
        if not pairs:
            return
        best = None
        for pair in pairs[:8]:
            try:
                if asyncio.current_task().cancelled():
                    return
                r = await asyncio.wait_for(analyze_pair(client, pair), timeout=12)
                if r and len(r) == 3:
                    d, s, data = r
                    if d and s > 0:
                        if best is None or s > best["score"]:
                            best = {"pair": pair, "direction": d, "score": s, "data": data}
            except asyncio.CancelledError:
                raise
            except Exception:
                continue
        if best:
            pending_next_signal = best
            print(rainbow_text(f"🔍 Pre-analyzed next signal: {best['pair']} ({best['direction']})"))
    except asyncio.CancelledError:
        pass
    except Exception:
        pass
...
async def send_signal(client):
    ...
    # ── Fast path: use pre-analyzed signal if ready ──────────────────────
    global pending_next_signal
    if pending_next_signal:
        pre = pending_next_signal
        pending_next_signal = None
        best_pair  = pre["pair"]
        best_dir   = pre["direction"]
        best_data  = pre["data"]
        print(rainbow_text(f"⚡ 𝚄𝚜𝚒𝚗𝚐 𝚙𝚛𝚎-𝚊𝚗𝚊𝚕𝚢𝚣𝚎𝚍 𝚜𝚒𝚐𝚗𝚊𝚕: {best_pair} ({best_dir})"))
        current_signal_task = asyncio.create_task(
            asyncio.wait_for(process_signal(client, best_pair, best_dir, best_data), timeout=420)
        )
        signal_count += 1
        print(rainbow_text(f"✅ 𝚂𝚒𝚐𝚗𝚊𝚕 #{signal_count} 𝚜𝚎𝚗𝚝 𝚏𝚘𝚛 {best_pair} ({best_dir})"))
        return

    # ── Normal path: analyze pairs sequentially in the SAME event loop ──
