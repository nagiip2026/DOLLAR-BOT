...
def run_telegram_command_bot():
    """Run Telegram command bot — blocks the calling thread forever (for deploy mode).

    app.run_polling() in python-telegram-bot v20+ is a SYNCHRONOUS blocking call
    that manages its own event loop internally. Do NOT wrap it in asyncio.run() or
    loop.run_until_complete() — that creates a double-nested event loop and crashes.
    """
...
def _run_headless():
    ...
    # Run Telegram command bot in foreground — keeps the process alive on the server
